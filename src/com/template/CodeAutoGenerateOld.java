...
public class CodeAutoGenerateOld {
private static final Logger log = LoggerFactory.getLogger(CodeAutoGenerate.class);
	...
	public static void main(String[] args) throws Exception{
		...
		try{
			...
			String sql = "select table_name,table_comment from information_schema.tables where table_schema='cm-dispather'";
			...
			for(TemplateTable table : tables){
				new CodeAutoGenerateOld().build(table);
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
...
//		String packagePath = getPackageName(table.getTableName());
		...
		//生成Entity和Vo
		buildEntityAndVo(entityName, packagePath, table, filePath);
		//生成Dao
	...
	public void buildController(String entityName, String packagePath, String ftlPath, String comment, String filePath)throws Exception{
		...
		map.put("packagePath", packagePath);
		//生成接口
		generateFile(map, "controller.ftl", filePath + "/web/controller/"+entityName+"Controller.java");
	}
	 ...
	 * @throws Exception
	 */
	public void buildEntityAndVo(String entityName, String packagePath, TemplateTable table, String filePath)throws Exception{
		log.info("生成"+entityName+"Entity...");
		List<TemplateColumn> columns = getColumns(table.getTableName());
		Map<String,Object> map = new HashMap<String, Object>();
	...
	private void generateFile(Map<String,Object> map, String templateFile, String fileName) throws Exception{
		...
		config.setDirectoryForTemplateLoading(new File("templateOld"));
		config.setObjectWrapper(new DefaultObjectWrapper());
		//生成文件
		Template template = config.getTemplate(templateFile, Constant.fileEncoding);
		File file = new File("D:/cm-dispather/" + fileName);
	 ...
	 * @throws Exception
	 */
	private List<TemplateColumn> getColumns(String tableName) throws Exception{
		Connection conn = null;
